...
if use_lbfgsb
    ...
    relerr_trace = [];
    lbfgsbopts.errFcn = @(x) record_relerr(x);
...
if use_stoc
    ...
    info.nepoch = nepoch;
    info.relerr_trace = relerr_trace;
...
function stop = record_relerr(x)
    Mtmp = update(M0,1:nd,x);
    relerr_trace(end+1) = norm(X - full(Mtmp)) / normX;
    stop = false;
end
