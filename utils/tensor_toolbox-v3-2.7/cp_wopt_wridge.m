function [P, P0, output] = cp_wopt_wridge(Z,W,R,varargin)
...
params.addParameter('lambda', 0, @(x) x >=0);
...
if (isequal(funtype,'auto') && isa(Z,'tensor')) || isequal(funtype,'default')
    funhandle = @(x) tt_cp_wfun(Z,W,x,normZsqr, [], lambda);
else
...
function [f,G] = tt_cp_wfg(Z,W,A,normZsqr, lambda)
...
fridge = 0;
for i=1:length(A)
    fridge = fridge +norm(A{i},'fro')^2;
end
f = 0.5 * normZsqr - innerprod(Z,B) + 0.5 * norm(B)^2 + 0.5*lambda*fridge;
...
for n = 1:N
    G{n} = zeros(size(A{n}));
    G{n} = -mttkrp(T,A,n) + lambda*A{n};
end

function [f,g] = tt_cp_wfun(Zdata,W,x,normZsqr,memflag, lambda)
...
if isa(Zdata,'tensor') || isa(Zdata,'sptensor')
    ...
    [f,G] = tt_cp_wfg(Zdata,W,A,normZsqr, lambda);
else
    if ~exist('normZsqr','var')
